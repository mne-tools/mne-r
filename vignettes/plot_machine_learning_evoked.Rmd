...
In this example we are going to use the machine learning functionality of MNE-Python and scikit-learn
to analyze evoked responses.
...
epochs <- mne$Epochs(raw = raw, events = events, event_id =event_id,
                     tmin = tmin, tmax = tmax,
                     picks = picks %>% as.integer(), proj = T,
                     baseline = baseline, reject = NULL, decim = decim, preload = T)
```
...
preproc <- reticulate::import("sklearn.preprocessing", convert = F)
pipeline <- reticulate::import("sklearn.pipeline", convert = F)
linear_model <- reticulate::import("sklearn.linear_model", convert = F)
...
scores <- mne$decoding$cross_val_multiscore(time_decod, X, y, cv = 5L, n_jobs = 1L)
```
...
scores_df <- scores %>%
  as.data.frame() %>% gather(key = "time", value = "score")

# and add the info about the folds.
scores_df$fold <- rep(1:5 %>% sprintf("fold %s", .),
                      scores_df$time %>% unique() %>% length())
  
ggplot(
  data = scores_df,
  mapping = aes(x = as.numeric(time), y = score)) +
  geom_line(mapping = aes(group = fold), alpha = 0.4) +
  stat_summary(fun.y= "mean", geom = "smooth", size = 1.3, color = "red") +
