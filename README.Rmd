...
- Currently, if both 32-bit and 64-bit versions of R are installed, it seems devtools tries to build for both of them. This yields an ```loading failed for 'i386'``` error. To force building the package for your currently running R version use the ```INSTALL_opts``` argument of ```install_github```:
